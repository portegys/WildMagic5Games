...
Castle::Castle()
   :
     WindowApplication3("SpanTracker Castle Demo", 0, 0, 800, 600,
                        Float4(0.6f, 0.851f, 0.918f, 1.0f)),
     mTextColor(1.0f, 1.0f, 1.0f, 1.0f)
{
   Environment::InsertDirectory(WM5Path + "Projects/Castle/Geometry/");
   Environment::InsertDirectory(WM5Path + "Projects/Castle/Shaders/");
   Environment::InsertDirectory(WM5Path + "Projects/Castle/Textures/");
...
bool Castle::OnInitialize()
{
   ...
   mSpanTracker = new SpanTracker(this, WM5Path + "Projects/Castle/GUI/");

   MoveForward();
   mTrnSpeed *= (mTrnSpeedFactor * 2.0f);
   mRotSpeed *= (mRotSpeedFactor * 7.0f);
...
void Castle::OnIdle()
{
   ...
   if (mRenderer->PreDraw())
   {
      ...
      mSpanTracker->Draw(GetWidth(), GetHeight());
...
{
   if (mSpanTracker->IsHelping())
   {
      mSpanTracker->ToggleHelp();
      return(true);
   }
   ...
   case 'l':
      mSpanTracker->ToggleLogging();
      return(true);

   case 'r':
      if (WindowApplication3::OnKeyDown(key, x, y))
      {
         mSpanTracker->RotationRateChange(mRotSpeed);
         return(true);
      }
      break;
   ...
   case 's':
      mSpanTracker->SignalNoiseRatioDecrease();
      return(true);

   case 'S':
      mSpanTracker->SignalNoiseRatioIncrease();
      return(true);

   case 't':
      if (WindowApplication3::OnKeyDown(key, x, y))
      {
         mSpanTracker->MovementRateChange(mTrnSpeed);
         return(true);
      }
      break;
 ...
 * See: WindowApplication3::MoveCamera()
 * if (mUArrowPressed) MoveForward();
 * if (mDArrowPressed) MoveBackward();
 * if (mHomePressed) MoveUp();
 * if (mEndPressed) MoveDown();
 * if (mLArrowPressed) TurnLeft();
 * if (mRArrowPressed) TurnRight();
 * if (mPgUpPressed) LookUp();
 * if (mPgDnPressed) LookDown();
 * if (mInsertPressed) MoveRight();
 * if (mDeletePressed) MoveLeft();
 */

//----------------------------------------------------------------------------
bool Castle::OnSpecialKeyDown(int key, int x, int y)
{
   return(WindowApplication3::OnSpecialKeyDown(key, x, y));
}


//----------------------------------------------------------------------------
bool Castle::OnSpecialKeyUp(int key, int x, int y)
{
   mSpanTracker->UpdateDevicePosition();
   return(WindowApplication3::OnSpecialKeyUp(key, x, y));
}


//----------------------------------------------------------------------------
bool Castle::OnMouseClick(int button, int state, int x, int y, unsigned int)
{
   // SpanTracker GUI click?
   if (mSpanTracker->OnMouseClick(button, state, x, y))
   {
      return(true);
   }
...
void Castle::AdjustVerticalDistance()
{
   ...
   if (mPicker.Records.size() > 0)
   {
      ...
      mSpanTracker->AdjustVerticalDistance(mCamera->GetPosition().Z() - z);
   }
}
...
//----------------------------------------------------------------------------
void Castle::TurnLeft()
{
   WindowApplication3::TurnLeft();
   mSpanTracker->TurnLeft();
}


//----------------------------------------------------------------------------
void Castle::TurnRight()
{
   WindowApplication3::TurnRight();
   mSpanTracker->TurnRight();
}
...
//----------------------------------------------------------------------------
void Castle::LookDown()
{
   WindowApplication3::LookDown();
   mSpanTracker->LookDown();
}
...
void Castle::MoveRight()
{
   // No-op.
}


// Perform automatic SpanTracker measurement.
void Castle::AutoSpanTrack()
{
   default_random_engine generator;

   srand((unsigned int)time(0));
   generator.seed((unsigned long)rand());
   uniform_int_distribution<int> iteration_distribution(MIN_AUTO_ACTIONS, MAX_AUTO_ACTIONS);
   int n = iteration_distribution(generator);
   mSpanTracker->Reset();
   if (!mSpanTracker->TrackStartTarget())
   {
      return;
   }
   mSpanTracker->startTargetTrackCheck->setChecked(true);
   mSpanTracker->startTargetIsTracked = true;
   mSpanTracker->UpdateDevicePosition();
   uniform_int_distribution<int> action_distribution(0, 5);
   for (int i = 0; i < n; i++)
   {
      switch (action_distribution(generator))
      {
      case 0:
         MoveForward();
         break;
   ...
   if (!mSpanTracker->TrackEndTarget())
   {
      return;
   }
   mSpanTracker->endTargetTrackCheck->setChecked(true);
   mSpanTracker->endTargetIsTracked = true;
   APoint position = mSpanTracker->GetTargetPosition();
   memset(mSpanTracker->positionBuf, 0, BUFSIZ);
   sprintf(mSpanTracker->positionBuf, "%.2f %.2f %.2f", position.X(), position.Y(), position.Z());
   int len = strlen(mSpanTracker->positionBuf);
   for (int i = len; i < 26; i++)
   {
      mSpanTracker->positionBuf[i] = ' ';
   }
   mSpanTracker->targetPositionText->setLabelString(mSpanTracker->positionBuf);
   float distance = mSpanTracker->GetDistance();
   memset(mSpanTracker->distanceBuf, 0, BUFSIZ);
   sprintf(mSpanTracker->distanceBuf, "%.2f", distance);
   len = strlen(mSpanTracker->distanceBuf);
   for (int i = len; i < 16; i++)
   {
      mSpanTracker->distanceBuf[i] = ' ';
   }
   mSpanTracker->targetDistanceText->setLabelString(mSpanTracker->distanceBuf);
}
